...
SCRANTIC::Command SCRANTIC::ADSFile::getNextCommand(u_int16_t movie, int32_t ttm, int32_t scene)
{
    std::map<u_int16_t, std::vector<Command> >::iterator it;

    it = script.find(movie);
    if (it  == script.end())
    {
        Command cmd;
