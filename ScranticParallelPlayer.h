#ifndef SCRANTICPARALLELPLAYER_H
#define SCRANTICPARALLELPLAYER_H
...
struct TTMScene
{
    ...
    bool wantsInit = false;
    bool newScene = true;

    std::pair<u_int16_t, u_int16_t> currentColor;
    u_int8_t currentImgSlot = 0;
    u_int8_t currentPalSlot = 0; // unused
    ...
    //freed by SCRFile
    SDL_Texture *currentScreen = NULL;

    bool saveImage = false;
    bool saveNewImage = false;

};

struct RendererState
{
    bool anyTTMSceneRunning;
    bool saveImage;
    bool saveNewImage;
    bool displayMenu;
    bool clear;
    ...
    //std::list<TTMScene> currentScenes;
...
class ScranticParallelPlayer
{
protected:
    bool lookForLabel;
    u_int16_t labelRes;
    u_int16_t labelScene;
...
public:
    ScranticParallelPlayer(std::string ResMap, std::string ScrExe);
    void AdvanceScript();
    void render();
    ...
    u_int32_t getCurrentDelay();
    ~ScranticParallelPlayer();
};

}

#endif // SCRANTICPARALLELPLAYER_H
